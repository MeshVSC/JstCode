...
export default function ProjectUploader({ onProjectUpload, onShowTemplates, isAnalyzing }: ProjectUploaderProps) {
  ...
  return (
    <div
      {...getRootProps()}
      className={`
        rounded-lg p-6 text-center cursor-pointer transition-all bg-elevated
        ${isDragActive
          ? 'bg-hover'
          : 'hover:bg-hover'
        }
      ...
      <div className="space-y-3">
        <div className="flex items-center justify-center h-12">
          {isLoading ? (
            ...
            <Image src="/blocks_W.png" alt="Drop files" width={40} height={40} className="opacity-80" style={{filter: 'var(--logo-filter, none)'}} />
          ) : (
            <Image src="/blocks_W.png" alt="Upload files" width={32} height={32} className="opacity-60" style={{filter: 'var(--logo-filter, none)'}} />
          )}
        </div>
